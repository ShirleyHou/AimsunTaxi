...
testID = ?
DiscreteInterval : the interval of sample time. Currently set to 2 minutes. Assume in Aimsun simualtion step is 1 sec. The sampling time should also be set to same as DiscreteInterval.
sinscale : to managing the variation of vacant/waiting passenger. smaller sinscale (<200) corresponds to quicker variation rate (not desirable) 
...
'''
Information storage, initial values for passengers and taxis
'''
Passengerlist = []
Taxilist = {}
i = 0
...
unassignedAccumulation = 0
vacantTimeSteps = []
waitingTimeSteps = []
unassignedTimeSteps = []
totalTaxiTimeSteps = []
deletedPassengerTimeSteps = []
...
class Taxi:
    ...
    def __init__(self, occupy, passengerid, origin, destination, path, stoptime, firststop, stopping, sectionlength,
                 midsection, i):
        self.occupy = occupy
...
BigmodelFileName = "network_withlength_largeModel.txt"
SmallModelFileName = "network_withlength.txt"
BoxhillFileName = "network_withlength_boxhill.txt"
f = open(BigmodelFileName)  # netowrk connectivity data input, from Aimsun script.
...
for x in innerSec.keys():
    for y in innerSec.keys():
        if innerSec[x][1] == innerSec[y][0] and innerSec[x][0] != innerSec[y][1]:
            ...
            mid2mid_length = (secLength[x] + secLength[y])/float(2)
...
for x in inSec.keys():  # exclude title: ['Section ID', ' Origin Node', ' Destination Node']
    for y in range(0, len(secIDs)):
        ...
        if dNode == oNode:
            ...
            # this.origin != other.destination: just to make sure they are not same one in reverse.
            G.add_node(x)
            mid2mid_length = (secLength[x] + secLength[secIDs[y]])*0.5
...
def AAPIManage(time, timeSta, timeTrans, acycle):
    ...
    try:
        ...
        global accumulation,accumulations, flow, flows, completion, completions, sectionid, junctionid, totallength, DiscreteInterval,\
            clock, Passengerlist, Taxilist, CARS, meeting, meetingCount, waitAccumulation, vacantAccumulation, waitingTimeSteps, vacantTimeSteps, totalTaxiTimeSteps, \
            unassignedTimeSteps, deletedPassengerTimeSteps, unassignedAccumulation, sinscale
        ...
        for c in Taxilist.keys():
            ...
            if Taxilist[c] == None:
                continue
            InfVeh = AKIVehTrackedGetInf(c)
            #print("Taxi", c, "position", InfVeh.idSection, "is currently assigned with", Taxilist[c].passengerid,
                  #" and Has completed the trip of", Taxilist[c].completedPassenger)
            ...
            if len(Taxilist[c].path) != 0:
                ...
                if InfVeh.idSection == -1:
                    ...
                    if AKIVehTrackedModifyNextSection(c, int(Taxilist[c].path[Taxilist[c].i])) != 0:
                        print('Error_____________2 :   Taxi', c, 'Currently on', InfVeh.idSection,
                              ' has error modifying to the correct next step',
                              Taxilist[c].path[Taxilist[c].i])
            ...
            if (Taxilist[c].passengerid == 0 or len(
                    Taxilist[c].path) == 0) and InfVeh.idSection != -1:  # case no customer.
            ...
            if len(Taxilist[c].path) != 0 and InfVeh.idSection == int(Taxilist[c].path[-1]):
                ...
                if clock - Taxilist[c].stoptime > 3 and Taxilist[c].stopping == True:  # if taxi restarts.
                    ...
                    if Pickup:
                        ...
                        print('Taxi', c, 'dropped off passenger', Taxilist[c].passengerid,
        ...
        deletedPassengerTimeSteps.append(deletedPassenger)
        print ('Number of passenger unassigned: ', unassigned)
        print('Number of passenger deleted: ', deletedPassenger)
        unassignedTimeSteps.append(unassigned)
        ...
        totalTaxiTimeSteps.append(totalTaxi)

        unassignedAccumulation += unassigned
        ...
        def decision(time, sinscale):
            import random
            return random.random() < np.sin(float(time)/float(sinscale))

        #decrease this factor to increase passenger demand. clock % n
        if clock % (sinscale/60) == 0 and decision(clock+sinscale/2, sinscale): #introduces new passenger every DI sec.
        ...
        #increase this factor to decrelerate taxi deletion. wait * n
        if vacant> wait*3 and unassigned ==0 :
            for c in Taxilist.keys():
        ...
        if clock % DiscreteInterval == 0:
            ...
            meeting.append([meetingCount, float(waitAccumulation)/float(DiscreteInterval), float(vacantAccumulation)/float(DiscreteInterval),float(unassignedAccumulation)/float(DiscreteInterval)])
            ...
            for sectionnumbmer in sectionid:
                numbersinsection = AKIVehStateGetNbVehiclesSection(sectionnumbmer,
...
def AAPIFinish():
    ...
    try:
        ...
        file.write("meetingCount\t wait\t vacant\t unassigned\t Accumulation\t Flow\t Completion\n")
        print('len(flows)', len(flows))
        print('len(accumulations)', len(accumulations))
        print('len(completions)', len(completions))

        for v in range(0, len(flows)): #because flows will be smaller than the other datas due to it has 1 sec delay time.
        ...
        file.write("Vacant\t Waiting\t Unassigned\t Deleted\t TotalTaxi\n")
        for i in range(0, len(vacantTimeSteps)):
            file.write(str(vacantTimeSteps[i])),
            file.write(',')
            file.write(str(waitingTimeSteps[i])),
            file.write(',')
            file.write(str(unassignedTimeSteps[i])),
            file.write(',')
            file.write(str(deletedPassengerTimeSteps[i])),
            file.write(',')
            file.write(str(totalTaxiTimeSteps[i])),
            file.write('\n')
...
def AAPIEnterVehicle(idveh, idsection):
    ...
    try:
        ...
        if len(
                Taxilist) <= CARS or clock % (DiscreteInterval*2) == 0:  # and not CARcondition : #only introduce tracked vehicle when it enters connected G.
